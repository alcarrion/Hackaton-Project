...
// Movemos questions dentro del componente para evitar el error de ESLint
const Test = () => {
  const questions = [
    {
      id: 1,
      ...
      {result ? (
        <div className={`result-container ${result.toLowerCase()}`}>
          ...
          <p className="result-description">
            {result === "Gryffindor" && "¡La casa de los valientes y audaces!"}
            {result === "Hufflepuff" && "¡La casa de los leales y trabajadores!"}
            {result === "Ravenclaw" && "¡La casa de los sabios e ingeniosos!"}
            {result === "Slytherin" && "¡La casa de los astutos y ambiciosos!"}
          </p>
          <button
